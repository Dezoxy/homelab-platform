#!/usr/bin/env bash
# Run `terraform init` + `terraform plan` for one Proxmox infra stack.
#
# Used by `make terraform-plan TARGET=<host>` and `make terraform-plan-all`.
# Expects to be invoked under the secret wrapper (scripts/dev/azure_kv_run.sh),
# which exports the upstream secret names (PROXMOX_API_URL, PROXMOX_TOKEN_ID, …).
# This script remaps them to the TF_VAR_* / TF_TOKEN_* names the providers expect
# — same mapping that scripts/ci/export_runtime_env.sh does for CI.
#
# Args:
